...
export const BookmarksContextProvider = ({ children }) => {
    ...
    const tagsFunc = {
        ...
        deleteTags: async function () {
            totTags.map(async ttd => {
                if (ttd.ischeck === true) {
                    const response = await fetch('https://bmsaver-server.herokuapp.com/api/tags/' + ttd._id, {
                        method: 'DELETE',
                        headers: {
        ...
        addTags: async function (inputText) {
            ...
            const response = await fetch('https://bmsaver-server.herokuapp.com/api/tags', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const bookFunc = {
        addBookmark: async function (bookToAdd) {
            ...
            const response = await fetch('https://bmsaver-server.herokuapp.com/api/bookmarks', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
        ...
        modifyBookmark: async function (bookUpdated) {
            ...
            const response = await fetch('https://bmsaver-server.herokuapp.com/api/bookmarks/' + bookUpdated._id, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`
            ...
            if (response.ok) {
                ...
                const upd = await fetch('https://bmsaver-server.herokuapp.com/api/bookmarks/', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${user.token}`
        ...
        deleteBookmark: async function (bookToDel) {
            const response = await fetch('https://bmsaver-server.herokuapp.com/api/bookmarks/' + bookToDel._id, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`
